...
then move on to [Reading JSON](reading.md) and [Writing JSON](writing.md) for
...
-   :material-book-open-variant: **Learn the API**
    ...
    [:octicons-arrow-right-24: Reading JSON](reading.md)
...
- An alist becomes an object only after explicit conversion with
  [`alist->json-object`](conversion.md).
...
- **Bounded by default.** Every reader and writer entry point enforces finite
  [size and depth limits](security.md) suited to untrusted input.
...
See [Data Model and Mapping](data-model.md) for the full rules, including the
